  ...
  <style>
    ...
    * {
    ...
    select {
      ...
      font-family: inherit;
      /* 폰트 상속 */
      background: url("img/caret-down-outline.png") no-repeat 95% 50%;
      /* 네이티브 화살표 대체 */
  ...
  <body>
    <div class="wrapper">
      ...
      <div class="display-container">
        <h1 style="font-size: 8rem">오류가 발생했습니다!</h1>
        <h3 style="font-size: 4rem">404 Page Not Found!</h3>
      </div>
    </div>
  </body>
</html>
...
io.on("connection", (socket) => {
  ...
  socket.on("on_chat", (data) => {
    ...
    io.emit("on_chat", param);
    Logging(param.name, param.msg, param.time);
  });

  socket.emit("on_log", LogToHtml());
});

function LogToHtml() {
  var html = "";
  for (index = 0; index < chatLog.length; index++) {
    if (chatLog[index].name == undefined && chatLog[index].msg == undefined) {
      html += `<div style="text-align: center;">새로운 유저가 접속했습니다</div>`;
      continue;
    }
    html += `<li class="received">
    <span class="profile">
    <span class="user">${chatLog[index].name}</span>
    ...
    <span class="message">${chatLog[index].msg}</span>
    <span class="time">${chatLog[index].time}</span>
    </li>`;
  }
  return html;
}
