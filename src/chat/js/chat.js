...
function OnChat(data) {
  ...
  AddTextModel(name, msg, time);
  ScrollDownEnd();
}

function UserJoin() {
  const div_join = document.createElement("div");
  div_join.innerText = "새로운 유저가 접속했습니다";
  div_join.classList.add = "sent";
  div_join.classList.add = "received";
  div_join.style.cssText = "text-align: center";
  detectBottom();
  chatlist.appendChild(div_join);
  ScrollDownEnd();
}
...
function AddTextModel(name, msg, time) {
...
function ScrollDownEnd() {
  if (isBottom) displayContainer.scrollTo(0, displayContainer.scrollHeight);
}
...
function Start() {
  ...
  $.post(
    "/api/v1/chatlog",
    { room: roomName, nickname: nickname },
    function (data) {
      ReciveLog(data);
      ScrollDownEnd();
